...
import { FaTimes, FaCheckCircle, FaCrown, FaArrowRight, FaSpinner, FaShieldAlt } from 'react-icons/fa';
...
export const UpgradePlanModal = ({ 
  ...
  // Fetch plans and current status
  useEffect(() => {
    if (isOpen) {
      loadData();
    }
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      // Fetch plans with current currency
      const plansRes = await billingApi.getPlans(currency);
      setPlans(plansRes.data?.plans || []);

      // Try to get current plan from localStorage or session (for logged-in users)
      const token = localStorage.getItem('dealdesk_token');
      if (token) {
        try {
          const statusRes = await billingApi.getStatus();
          setCurrentPlan(statusRes.data?.planId || null);
        } catch {
          setCurrentPlan(null);
        }
      }
      ...
      <div className="upgrade-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="upgrade-modal-header">
          <div className="upgrade-modal-header-left">
            ...
            <div>
              <h3>{title}</h3>
              <p className="upgrade-modal-subtitle">{message}</p>
            </div>
          </div>
        ...
        <div className="upgrade-billing-toggle">
          ...
          <button
            className={`upgrade-toggle-btn ${billingCycle === 'annual' ? 'active' : ''}`}
            onClick={() => setBillingCycle('annual')}
          >
            Annual <span className="upgrade-save-badge">Save 17%</span>
          </button>
        </div>
          ...
          {loading ? (
            ...
            plans.map((plan) => {
              ...
              const price = billingCycle === 'annual' 
                ? (plan.annualPriceUSD || plan.monthlyPriceUSD * 10) 
              ...
              return (
                ...
                >
                  ...
                  <div className="upgrade-plan-price">
                    ...
                    {price > 0 && <span className="upgrade-plan-period">/ {billingCycle === 'annual' ? 'year' : 'month'}</span>}
                  </div>
        ...
        <div className="upgrade-modal-footer">
          <p><FaShieldAlt className="upgrade-footer-icon" /> Secure payment via Razorpay. 100% encrypted.</p>
